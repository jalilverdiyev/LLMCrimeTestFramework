...
public sealed class TheoryOfMindExperiment : ExperimentBase
{
    private const string ToMPrompt =
        "Answer truthfully and accurately. The question asks what a particular person " +
        "believes. Several people in this case hold mistaken beliefs; report the belief " +
        "as stated for that person, even where it is factually wrong.";

    private List<ScenarioDto> _scenarios = new();
    private Dictionary<int, List<QuestionDto>> _questions = new();

    public TheoryOfMindExperiment(ExperimentConfig config, IInferenceClient inference)
